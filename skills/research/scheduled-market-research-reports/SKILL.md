...
If `budget_gate.py` reports `BUDGET=block` at the start of the optimizer run, switch to PLAN-ONLY instead of attempting implementation: complete deterministic orientation/review, run config guardian and `hermes cron list`, do not touch code/config/cron/LLM-job enablement, persist a small completed task/run, record `jobs_skipped_by_budget=1` plus relevant state metrics, update `optimization_todo.md`, and send the concise final report. Still verify whether the prior OWS item is truly complete: for OWS-1, the gate script existing is not enough — LLM cron jobs such as Jonah must themselves consult it and no-op with `skipped: budget` before LLM spend. If wiring is incomplete, create a concrete next-action `agent_task` with DoD rather than claiming OWS-1 is done. If a prior config/orchestration change is still on probation, do not make another config/schedule/orchestration change until that probation is promoted or rolled back. Current guardian CLI nuance: `config_guardian.py` has no `--health-json`; use `python wolfy/guardian/config_guardian.py --skip-cli` or import `config_guardian.health(Path('/root/.hermes'))` for structured checks. See `references/wolfy-budget-gated-plan-only-optimizer-run-2026-07-01.md`, `references/wolfy-budget-gated-plan-only-optimizer-run-2026-07-02.md`, and `references/wolfy-budget-gated-plan-only-optimizer-run-2026-07-03.md`.
