...
## Mike and Clerky role split

When the user wants non-market operations separated from Wolfy, use these profiles:

- `mike`: IT/admin operations for LLM usage-limit monitoring, storage, Postgres coordination hygiene, embedding sync infrastructure, aggregate usage accounting, and maintenance alerts. Mike does not do market analysis or trade recommendations.
- `clerky`: administrative tracking for Kanban hygiene, bounded work allocation, status/progress ledgers, handoffs, and dependency cleanup. Clerky does not do market analysis or infrastructure changes unless a card explicitly asks.

Cron ownership pattern established:

- Mike owns no-agent ops jobs by name/function: usage-limit watchdog, storage watchdog, Postgres stale coordination watchdog, aggregate usage snapshots, and Postgres knowledge embedding sync. For script-only `no_agent=true` jobs, do not set a Hermes profile unless the wrapper exists under that profile's `scripts/` directory; profile-scoped cron resolves relative scripts under the profile home and can fail with `Script not found: /root/.hermes/profiles/<profile>/scripts/...`. Either clear the profile for root `~/.hermes/scripts/` wrappers or sync wrappers into the profile scripts dir.
- Clerky owns Kanban/admin jobs: bounded work allocator and four-hour progress ledger. The same profile-script caveat applies to Clerky no-agent jobs.

Kanban assignee pattern:

- Infrastructure/data plumbing cards -> `mike`.
- Kanban/status/tracking cards -> `clerky`.
- Technical entry/exit cards -> `yang`.
- Market alpha/recommendation cards stay with Wolfy/default unless a dedicated Wolfy profile is later created.
