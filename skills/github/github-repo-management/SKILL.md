...
## Safe source-code import before first push

When putting an existing operational directory into GitHub, do not blindly `git add .`. First separate source code from runtime state and secrets:
...
git init -b main
git add .gitignore '*.py' '*.md' '*.sql' config/*.json tests/*.py 2>/dev/null || true
python3 - <<'PY'
...
if hits:
    print('SECRET_SCAN_HITS')
    for f,i,n in hits:
        print(f'{f}:{i}:{n}')
    sys.exit(1)
print(f'SECRET_SCAN_OK staged_files={len(files)}')
PY
