...
function render(d){
  ...
  polls.innerHTML=(d.polls||[]).map(p=>`<div class="rec"><b>${p.question}</b><div>${(p.choices||[]).map(c=>`<button onclick="answerPoll('${p.id}','${String(c).replaceAll("'","&#39;")}')">${c}</button>`).join('')}</div></div>`).join('')+((d.poll_answers||[]).length?`<h3>Current answers</h3>${(d.poll_answers||[]).slice(0,5).map(a=>metric(a.poll_id,a.choice)).join('')}`:'')||'<div class="muted">No polls</div>';
